...
import { PhotosToAdd } from './PhotosInterfaces';
...
export class PhotosAdd {
    ...
    // File selection
    fileSelection(): void {
        photosDomElements.customFileInput.addEventListener('change', (e) => {
    ...
    showThumbs(fileSize: number, fileLastModified: number, file: File, i: number): void {
        ...
        reader.addEventListener('load', () => {
            ...
            photosDomElements.output.insertBefore(div, null);
        }, false);
    ...
    // To send
    toSend(): void {
        photosDomElements.btnSubmit.addEventListener('click', (e) => {
            ...
            const photosToAdd: PhotosToAdd[] = [];

            for (let i = 0; i < mediaFiles.length; i++) {
                if (photosToAdd !== undefined) {
                    photosToAdd[i] = { name: fileNames[i], isCover: checkBoxes[i] };
                }
            }

            // dataToAdd to JSON
            const dataToAddJson = JSON.stringify(photosToAdd);
